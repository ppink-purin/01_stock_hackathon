...
> 📝 **한 글자(토큰)씩** 답변을 만들고, 🎲 **확률**로 다음 글자를 선택해요!
...
> 💡 API Key가 노출되면 다른 사람이 내 비용으로 AI를 사용할 수 있어요!
> 반드시 `.env.local` 금고에 보관하세요 🔒
...
| 방식 | 비유 | 결과 |
|:---|:---|:---|
| ❌ 바로 답변 | 풀이 없이 답만 적기 | 틀릴 확률 높음 |
| ✅ 단계적 사고 | 풀이 과정 보여주기 | 정확도 향상! |
...
> 💡 **좋은 지침서 = 좋은 서비스 품질!** 규칙이 구체적일수록 답변이 일관되어요 ✨
...
- 📝 **용도**를 명확하게 작성하세요
- 🔗 **언제 사용**해야 하는지 조건을 적으세요
- 📊 **입출력 형식**을 설명해주세요

> 🎯 설명서가 구체적일수록 비서가 도구를 **정확한 타이밍**에 사용해요!
...
📱 브라우저 ←───────── 🖥️ 서버
  ...
  data: {"type":"tool_call","name":"시세 조회"}  ← 도구 사용!
  data: {"type":"done","text":"...전체..."}      ← 완료!
```
...
```
📋 시스템 프롬프트 (route.ts)       ✂️ 클라이언트 파싱
─────────────────────────           ─────────────────────
## 🔍 의도 분석         ──regex──>  📁 접기/펼치기
## 📋 탐색 계획         ──regex──>  📁 접기/펼치기
## 💡 최종 답변         ──regex──>  💬 메인 말풍선
[추천질문: A | B | C]   ──regex──>  🔘 버튼 3개
```
...
> 💡 Haiku는 Opus보다 **15배 저렴**하면서 Tool Use에 충분한 성능!

---

# 🔗 모델별 월간 비용 비교 (일 1,000건)
...
> 🎯 같은 기능이라도 모델 선택에 따라 비용이 **15배** 차이 나요!
...
- ⚡ **속도** -- 주식 정보는 빠른 응답이 생명!
- 💰 **비용** -- 많은 사용자, 자주 질문 = 저렴해야!
- ✅ **충분한 성능** -- Tool Use + 한국어에 Haiku로 충분
- 🔧 **도구가 보완** -- AI 한계를 외부 API가 메움
...
Level 1: 🔧 도구 고장     → 비서가 안내
Level 2: 📞 전화 끊김     → 서버가 에러 신호
Level 3: 🌐 인터넷 끊김   → 브라우저에서 표시
```
...
> ⚠️ 에러가 나도 `controller.close()`를 반드시 호출해야 해요!
...
> 🎯 이 전체 흐름이 **하나의 질문**에서 일어나는 일이에요!
> 사용자 → 서버 → Claude → 도구 → 스트리밍 → 화면!
...
> 🎉 이 10가지 비유를 기억하면 AI 개발의 핵심을 이해한 거예요!
